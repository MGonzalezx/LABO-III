...
window.addEventListener("click", (e)=>{
  ...
  if(e.target.matches("td")){ 
      ...
      //deleteAnuncioAxios(parseInt($formulario.txtId.value));
...
const $formulario = document.forms[0];
  ...
  $formulario.addEventListener("submit", (e) => {
    ...
    if(formAnuncio.id === ''){
        ...
        //createAnuncioAxios(formAnuncio);
        ...
        //updateAnuncioAxios(formAnuncio);
...
const createAnuncioAxios = (nuevoAnuncio)=>{
  axios(URL,{
    method:"POST",
    header:{
...
const updateAnuncioAxios = (anuncioEditado)=>{
 
  axios(URL + "/" + anuncioEditado.id,{
      method:"PUT",
      header:{
          "Content-Type":"application/json"
      },
      data:JSON.stringify(anuncioEditado),
  })
  .then(({data})=>{
      console.log(data);
  })
